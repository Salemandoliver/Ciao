...
export function SecurityTab({ onSignedOut }: { onSignedOut: () => void }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const res = await api<{ items: Session[] }>(
        `/v1/biz/auth/sessions${refresh ? `?current=${encodeURIComponent(refresh)}` : ""}`,
      );
  ...
  async function revoke(id: string) {
    try {
      await api(`/v1/biz/auth/sessions/${id}`, { method: "DELETE" });
      await load();
    } catch {
      setError(c.failed);
    }
  ...
  async function revokeAll() {
    ...
    setBusy(true);
    try {
      const res = await api<{ count: number }>("/v1/biz/auth/sessions/revoke-all", {
        method: "POST",
        body: "{}",
      });
