 ...
 *
 * Deliberately not a copy of the marketplace's. The two apps sign in
 * differently — a guest proves a phone number with an OTP, a partner types a
 * password — and they must not be able to borrow each other's sessions. So:
 *
 *  - a different storage key, so the two never collide even if someone runs
 *    both on localhost during development;
 *  - a different refresh endpoint, backed by a different table;
 *  - a token minted with the `partner` audience, which the API refuses on
 *    every consumer route and vice versa.
 *
...
const REFRESH_KEY = "ciao_partner_refresh";
...
async function performRefresh(refresh: string): Promise<boolean> {
  let res: Response;
  try {
    res = await fetch(`${API_URL}/v1/partner/auth/refresh`, {
      method: "POST",
...
export async function signOut(): Promise<void> {
  ...
  if (refresh) {
    await fetch(`${API_URL}/v1/partner/auth/logout`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ refreshToken: refresh }),
    }).catch(() => undefined);
