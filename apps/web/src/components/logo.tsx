/**
 * The Greeting Bubble — design doc §3.3 (logo v4).
 * "ciao" in Baloo Bhaijaan 2 ExtraBold converted to vector paths
 * (font-independent), amber sun in place of the o, i-dot deliberately
 * chopped flush against the bubble's top edge, equal padding on all four
 * sides measured against the c-a-o x-height block.
 * Fixed viewBox ⇒ identical proportions at every size on every screen.
 */
...
const VB = { x: -14.48, y: -67.68, w: 216.62, h: 86.86 };
const TAIL_H = 20.84;
const RX = 26.06;
...
function Bubble({
  ...
  const h = withTail ? VB.h + TAIL_H : VB.h;
  const bubbleFill = reversed ? "#FFFFFF" : "#1B4F72";
  const textFill = reversed ? "#1B4F72" : "#FFFFFF";
  const clipId = `ciao-clip-${withTail ? "t" : "b"}-${reversed ? "r" : "p"}`;
  return (
    <svg
    ...
    >
      <defs>
        <clipPath id={clipId}>
          <rect x={VB.x} y={VB.y} width={VB.w} height={VB.h} rx={RX} />
        </clipPath>
      </defs>
      <rect x={VB.x} y={VB.y} width={VB.w} height={VB.h} rx={RX} fill={bubbleFill} />
      {withTail ? (
        <path
          d={`M 11.52 ${(VB.y + VB.h - 1).toFixed(2)} l 20.85 0 l -20.85 ${TAIL_H + 1} z`}
          fill={bubbleFill}
        />
      ) : null}
      <g clipPath={`url(#${clipId})`}>
        <path d={CIA_PATH} fill={textFill} />
        <circle cx="160.78" cy="-24.25" r="23.78" fill="#E8A33D" />
      </g>
    </svg>
  );
}
